...
def _advance_to_paper(registry: StrategyRegistry, key: str) -> StrategyVersion:
    registry.promote(key, PromotionStage.BACKTEST, occurred_at=APPROVED_AT)
    return registry.promote(key, PromotionStage.PAPER, occurred_at=APPROVED_AT)
...
class TestVersionImmutability:
    ...
    def test_stage_is_not_part_of_the_fingerprint(self, strategy_version):
        promoted = strategy_version.model_copy(update={"stage": PromotionStage.PAPER})
    ...
    def test_versions_must_be_registered_at_research_stage(self, strategy_version):
        ...
        with pytest.raises(PromotionAuthorityError, match="must be registered at"):
            registry.register(
                strategy_version.model_copy(update={"stage": PromotionStage.PRODUCTION})
            )


class TestPromotionRequiresHumanApproval:
    def test_production_without_approval_is_refused(self, strategy_version):
        registry = StrategyRegistry()
        registry.register(strategy_version)
        ...
        with pytest.raises(PromotionAuthorityError, match="requires a recorded human approval"):
            registry.promote(
                strategy_version.key, PromotionStage.PRODUCTION, occurred_at=APPROVED_AT
            )

    def test_production_with_approval_succeeds(self, strategy_version):
        registry = StrategyRegistry()
        registry.register(strategy_version)
        ...
        assert promoted.stage is PromotionStage.PRODUCTION
